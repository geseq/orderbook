...
type orderQueuePool struct {
	ch *itemChanOrderQueuePool
}

func newOrderQueuePool(maxSize uint64) *orderQueuePool {
	ch := newItemChanOrderQueuePool(maxSize)
	p := &orderQueuePool{
		ch: ch,
	}
...
type itemChanOrderQueuePool struct {
...
func newItemChanOrderQueuePool(size uint64) *itemChanOrderQueuePool {
	size = roundUpNextPowerOfTwoItemChanOrderQueuePool(size)
	return &itemChanOrderQueuePool{
		lastCommittedIndex: 0,
		nextFreeIndex:      0,
		readerIndex:        0,
		indexMask:          size - 1,
		contents:           make([]*orderQueue, size),
	}
}

// Put writes a CacheItem to the front of the channel
func (c *itemChanOrderQueuePool) Put(value *orderQueue) {
...
func (c *itemChanOrderQueuePool) Read() *orderQueue {
...
// Empty the channel
func (c *itemChanOrderQueuePool) Empty() {
